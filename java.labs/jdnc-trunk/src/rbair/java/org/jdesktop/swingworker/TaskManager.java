/*
 * $Id: TaskManager.java 726 2005-10-14 19:06:20Z rbair $
 *
...
public class TaskManager {
    ...
    // the next couple lines are hacked, for working without Application object
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                synchronized(tasks) {
                    for (int i=0; i<tasks.size(); i++) {
                        SwingTask t = (SwingTask)tasks.get(i);
                        if (t.isModal()) {
                            if (progressDlg == null) {
                                progressDlg = new ProgressDialogImpl(frame); //TODO hacked for use without Application object
                            }
     ...
     * @param p
     */
    public void addTask(SwingTask t) {
        synchronized(tasks) {
            tasks.add(t);
            ...
            if (!popupTimer.isRunning() && t.isModal()) {
                popupTimer.start();
            }
        }
    }
     ...
     */
    public SwingTask[] getTasks() {
        updateProgressables();
        return (SwingTask[])tasks.toArray(new SwingTask[tasks.size()]);
    }
     ...
     * @author Richard Bair
     */
    private final class ProgressDialogImpl extends JDialog {
        /**
         ...
         */
        private JPanel panel; //TODO needs to be JXPanel
        /**
         ...
         */
        ProgressDialogImpl(Frame parent) {
	    super(parent, "Tasks...");
            initGui();
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    for (int i=0; i<tasks.length; i++) {
                        ...
                        if (panel == null) {
                            ...
                            newPanels[i] = new ProgressPanel(t);
                        } else {
        ...
        private void initGui() {
            ...
            panel = new JPanel();//ScrollablePanel(); //TODO
//            panel.setScrollableTracksViewportWidth(true);
        ...
        private final class ProgressPanel extends JPanel {
             ...
             * @param p
             */
            ProgressPanel(SwingTask t) {
                if (t == null) {
                    throw new NullPointerException("SwingTask t cannot be null");
                }
                this.t = t;
                initGui();
            }
            ...
            private void initGui() {
                ...
                descriptionLabel = new JLabel(this.t.getDescription());
                descriptionLabel.setIcon(this.t.getIcon());
                add(descriptionLabel, gbc);
                
                gbc.gridy = 1;
                gbc.insets = new Insets(3, 5, 0, 5);
                pb = new JProgressBar(this.t.getMinimum(), this.t.getMaximum());
                pb.setValue(this.t.getProgress());
                pb.setIndeterminate(this.t.isIndeterminate());
                add(pb, gbc);
                ...
                cancelButton.setVisible(this.t.isCanCancel());
                ...
                gbc.weightx = 1.0;
                gbc.weighty = 1.0;
                statusLabel = new JLabel(this.t.getMessage());
            ...
            void updateGui() {
                descriptionLabel.setIcon(this.t.getIcon());
                if (pb.getValue() != this.t.getProgress()) {
                    pb.setValue(this.t.getProgress());
                }
                if (pb.isIndeterminate() != this.t.isIndeterminate()) {
                    pb.setIndeterminate(this.t.isIndeterminate());
                }
                cancelButton.setVisible(this.t.isCanCancel());
                statusLabel.setText(this.t.getMessage());
            }
