/*
 * $Id: JXSlider.java 2766 2008-10-09 15:32:38Z mattnathan $
 *
...
import static org.jdesktop.swingx.event.SliderChangeEvent.Type.MARKER_GROUP_ADDED;
import static org.jdesktop.swingx.event.SliderChangeEvent.Type.MARKER_GROUP_CHANGED;
import static org.jdesktop.swingx.event.SliderChangeEvent.Type.MARKER_GROUP_REMOVED;
...
/**
 * Defines an extension to the standard JSlider. This extension adds support for an 'infinite' range, markers which can
 * be placed on the slider track and an adjustable view of the data. Due to the inherent complexity of allowing an
 * extremely large number of markers within the model there is added abstraction around the mutation of these values.
...
public class JXSlider extends JXComponent {
    ...
    /**
     * Forwards events from the model to observers of this class.
     */
    ...
    /**
     * Get the minimum value for the model.
     *
     ...
     * rendered to the screen but this may be handled specifically by the look and feels ui delegate for this class.
     *
    ...
    /**
     * Set the mutator used to change the values of the model. This can be set to null and in this case signifies that
     ...
     * given value is still being adjusted (i.e. while dragging the thumb) and certain performance optimisations can be
     * implemented.
     *
    ...
    /**
     * Get whether this slider is editable. Editable state is determined by both the editable property and the mutator
     * property. No JXSlider can be editable if its MarkerMutator is null. If this method returns true this is no
     * guarantee that any markers within this slider are editable.
     *
     ...
     *
     * @param group The marker group to find.
    ...
    protected void fireSliderMarkerGroupAdded(int index0, int index1) {
        ...
        for (int i = listeners.length - 2; i >= 0; i -= 2) {
            if (listeners[i] == SliderChangeListener.class) {
                // Lazily create the event:
                if (event == null) {
                    event = new SliderChangeEvent(this, MARKER_GROUP_ADDED, index0, index1);
                }
    ...
    protected void fireSliderMarkerGroupRemoved(int index0, int index1) {
        ...
        for (int i = listeners.length - 2; i >= 0; i -= 2) {
            if (listeners[i] == SliderChangeListener.class) {
                // Lazily create the event:
                if (event == null) {
                    event = new SliderChangeEvent(this, MARKER_GROUP_REMOVED, index0, index1);
                }
    ...
    protected void fireSliderMarkerGroupChanged(int index0, int index1) {
        ...
        for (int i = listeners.length - 2; i >= 0; i -= 2) {
            if (listeners[i] == SliderChangeListener.class) {
                // Lazily create the event:
                if (event == null) {
                    event = new SliderChangeEvent(this, MARKER_GROUP_CHANGED, index0, index1);
                }
    ...
    /**
     * Add a change listener for notification of changes the selection of marker values. Null values will be ignored.
     *
    ...
    /**
     * remove the given change listener from notification of selection change. Null values will be ignored.
     *
