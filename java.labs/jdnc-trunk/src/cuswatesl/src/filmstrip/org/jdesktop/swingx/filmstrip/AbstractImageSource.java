...
public abstract class AbstractImageSource implements ImageSource {
	...
	private final Map<String, SoftReference<BufferedImage>> namedSoftCache = new HashMap<String, SoftReference<BufferedImage>>();
	private final Map<String, BufferedImage> namedHardCache = new HashMap<String, BufferedImage>();
	...
	public void createCachedImage(final String name, final int maxWidth,
		...
		this.createCachedImage(name, maxWidth, maxHeight, scaler, callBack, true);
	}
	...
	public void cacheImage(String name, BufferedImage image) {
		this.namedSoftCache.put(name, new SoftReference<BufferedImage>(image));
	}
	...
	public BufferedImage getCachedImage(String name) {
		...
		if (namedHardCache.containsKey(name)) return namedHardCache.get(name);
		if (! namedSoftCache.containsKey(name)) return null;
		return namedSoftCache.get(name).get();
	}
	...
	protected void cacheHard(String name, BufferedImage image) {
		this.namedHardCache.put(name, image);
	}
	
	protected void cacheSoft(String name, BufferedImage image) {
		this.namedSoftCache.put(name, new SoftReference<BufferedImage>(image));
	}
