...
/*
 * $Id: AbstractPainterX.java 1193 2007-04-03 16:20:57Z kleopatra $
 *
...
/**
 * Alternative Cache/validation handling/notification. Removed dirty property.
 * 
 ...
 * @author rbair
 */
public abstract class AbstractPainterX<T> extends AbstractBean implements Painter<T> {
    /**
...
//    private boolean dirty = false;
     ...
     * array</p>
     * 
     * PENDING: add property changeListener for filter property changes!
     * 
...
//        if (old != visible) setDirty(true); //not the most efficient, but I must do this otherwise a CompoundPainter
     ...
     * the painting routines will be called.</p> If a cache had been used
     * before calling this, a property with name "cacheCleared" and new value 
     * Boolean.TRUE will be fired.
     * 
     *
    ...
    public void clearCache() {
        BufferedImage cache = getCachedImage();
        if (cache != null) {
            cache.flush();
        }
        if (cachedImage != null) {
            cachedImage = null;
            firePropertyChange("cacheCleared", null, Boolean.TRUE);
        }
    }
     ...
     *  
     * @return the BufferedImage used as cache. Null if no cache is
     * in use.
     */
    protected BufferedImage getCachedImage() {
...
//    }
    ...
    public final void paint(Graphics2D g, T obj, int width, int height) {
        ...
        if (shouldUseCache() || filters.length > 0) {
            validate(obj);
            BufferedImage cache = getCachedImage();
            if (needsRefresh(cache, width, height)) {
...
//        setDirty(false);
    ...
    private boolean needsRefresh(BufferedImage cache, int width, int height) {
        return cache == null || cache.getWidth() != width || cache.getHeight() != height;
    }
}
