...
/**
 * Mostly c&p of DefaultRowSorter (needed because not extendable). Additional functionality:
 * 
 ...
 * @since 1.6
 */
public abstract class BasicRowSorter<M, I> extends RowSorter<M> {
    /**
    ...
    private boolean[] alwaysUseComparator;
    ...
    private int fixedStartRows;
    private int fixedEndRows;

    /**
     * Creates an empty <code>DefaultRowSorter</code>.
     */
    public BasicRowSorter() {
    ...
    public void setFixedStartRows(int fixedStartRows) {
        this.fixedStartRows = fixedStartRows;
        allRowsChanged();
    }
    
    public void setFixedEndRows(int fixedEndRows) {
        this.fixedEndRows = fixedEndRows;
        allRowsChanged();
    }
    ...
    public void setComparator(int column, Comparator<?> comparator) {
        setComparator(column, comparator, false);
    }
    
    public void setComparator(int column, Comparator<?> comparator, boolean alwaysUseComparator) {
        checkColumn(column);
        if (comparators == null) {
            int count = getModelWrapper().getColumnCount();
            comparators = new Comparator[count];
            this.alwaysUseComparator = new boolean[count];
        }
        comparators[column] = comparator;
        this.alwaysUseComparator[column] = alwaysUseComparator;
    }
    ...
    private boolean include(int row) {
        if(row < fixedStartRows
                || row >= (modelRowCount - fixedEndRows)) {
            return true;
        }
    ...
    private int compare(int model1, int model2) {
        boolean model1FixedStart = model1 < fixedStartRows;
        boolean model2FixedStart = model2 < fixedStartRows;
        boolean model1FixedEnd = model1 >= (modelRowCount - fixedEndRows);
        boolean model2FixedEnd = model2 >= (modelRowCount - fixedEndRows);
        if((model1FixedStart && model2FixedStart)
                || (model1FixedEnd && model2FixedEnd)) {
            return model1 - model2;
        }
        else if(model1FixedStart) {
            return -1;
        }
        else if(model2FixedStart) {
            return 1;
        }
        else if(model1FixedEnd) {
            return 1;
        }
        else if(model2FixedEnd) {
            return -1;
        }
    ...
    private static class Row implements Comparable<Row> {
        private BasicRowSorter sorter;
        int modelIndex;

        public Row(BasicRowSorter sorter, int index) {
            this.sorter = sorter;
            modelIndex = index;
        }
