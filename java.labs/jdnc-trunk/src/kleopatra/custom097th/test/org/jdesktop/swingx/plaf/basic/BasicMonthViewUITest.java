/*
 * $Id: BasicMonthViewUITest.java 3312 2010-11-03 10:55:40Z kleopatra $
 *
...
//          test.runInteractiveTests();
        ...
        test.runInteractiveTests(".*Spinner.*");
      } catch (Exception e) {
          System.err.println("exception when executing interactive tests:");
          e.printStackTrace();
      }
  }
    public void interactiveSpinnerLayout() {
        // force loading of properties
        new JXMonthView();
        SpinnerModel model = new SpinnerNumberModel(2009, 0, 9999, 1);
...
//            }
        ...
        LOG.info("pref/min/max" + spinner.getPreferredSize() + "/" + spinner.getMinimumSize() + "/" + spinner.getMaximumSize());
        JLabel label = new JLabel("september");
        AbstractButton prev = createNavigationButton();
        prev.setIcon(UIManager.getIcon("JXMonthView.monthDownFileName"));
        AbstractButton next = createNavigationButton();
        next.setIcon(UIManager.getIcon("JXMonthView.monthUpFileName"));
        JComponent box = new JXPanel();
        box.setLayout(new BorderLayout());
        box.add(prev, BorderLayout.LINE_START);
        JComponent center = new JXPanel();
        center.add(label);
        center.add(spinner);
        box.add(center);
        box.add(next, BorderLayout.LINE_END);
...
//------------------------------
    ...
    public void testZoomableAddsCalendarHeader() {
        ...
        assertTrue("expected BasicCalendarHeader but was " + header.getClass(), 
                header instanceof BasicCalendarHeaderHandler.BasicCalendarHeader);
    }
