/*
 * $Id: JXTable.java 3145 2009-06-09 05:22:08Z rturnbull $
 *
...
import javax.swing.SortOrder;					// RT
...
// RT import org.jdesktop.swingx.decorator.SortOrder;
 ...
 * For Java 1.6
 * <p>
 * Enhanced Table component with support for general SwingX 
 ...
 * 
 * JXTable supports sorting and filtering of rows as per JTable. 
 * 
 * <p>
 ...
 * JXTableHeader.<br>
 * Use Shift-left click to remove sorting from a column
 * 
 * 
...
public class JXTable extends JTable implements TableColumnModelExtListener {
    ...
    // RT Added
    /**
     * required because JTable autoCreateRowSorter is private
     */
    protected boolean jxAutoCreateRowSorter;
...
// RT        // guarantee getFilters() to return != null
// RT        setFilters(null);
...
// RT end
    ...
    public void setModel(TableModel newModel) {
        ...
        try {
            ...
            // RT added
            if (jxAutoCreateRowSorter) {
        		setRowSorter(new JXTableRowSorter<TableModel>(dataModel));
        	}
       } finally {
...
// RT           selectionMapper = new DefaultSelectionMapper(filters,
//                    getSelectionModel());
            selectionMapper = new DefaultSelectionMapper(null,
            		getSelectionModel()); 					// RT
        }
...
// RT end
    ...
    @Override
    /**
     * Overridden to update column comparators and sortable options
	 * @see javax.swing.JTable#setRowSorter(RowSorter)	
     */
    public void setRowSorter(RowSorter<? extends TableModel> sorter) {
    	super.setRowSorter(sorter);
    	updateSorterFromColumns();
    }
    
    @Override
    public void setAutoCreateRowSorter(boolean autoCreateRowSorter) {
        boolean oldValue = jxAutoCreateRowSorter;
        jxAutoCreateRowSorter = autoCreateRowSorter;
        if (autoCreateRowSorter) {
            setRowSorter(new JXTableRowSorter<TableModel>(getModel()));
        }
    ...
    /**
     * Utility method replaces existing sort keys. If a RowSorter has not 
     * been applied, does nothing
     * 
     * @param column in view coordinates
     * @param sortOrder
     */
    public void setSortOrder(int column, SortOrder sortOrder) {
		...
		RowSorter.SortKey key = new RowSorter.SortKey(modelIndex, sortOrder);
		RowSorter.SortKey[] keys = new RowSorter.SortKey[] {key};
    ...
    public void toggleSortOrder(int column) {
    	...
    	if (sorter instanceof JXTableRowSorter) {
    		((JXTableRowSorter<? extends TableModel>)sorter).toggleSort(column);    		
    	} else {
	...
	public void removeSort(int column) {
        RowSorter<? extends TableModel> sorter = getRowSorter();
        if (sorter == null) return;
        if (!(sorter instanceof JXTableRowSorter)) return;
        column = convertColumnIndexToModel(column);
        ((JXTableRowSorter<? extends TableModel>)sorter).removeSort(column);
    }

    @SuppressWarnings("unchecked")
	private void updateSorterFromColumns() {
    	TableColumnModelExt model;
    	DefaultRowSorter sorter;
    	try {
			model = (TableColumnModelExt)getColumnModel();
			sorter = (DefaultRowSorter)getRowSorter();
		} catch (ClassCastException e) {
			return;
		}
		if (model == null || sorter == null) return;
		List<TableColumn> cols = model.getColumns(true);
		for (int i = 0; i < cols.size(); i++) {
			TableColumnExt column = (TableColumnExt)cols.get(i);
			if (column.getComparator() != null) {
				sorter.setComparator(i, column.getComparator());
			}
			sorter.setSortable(i, column.isSortable());
		}
    }
    
    /**
     * Hack to try and fool 1.5 version renderer
     * 
...
// RT        } else if (event.getPropertyName().equals("sortable")) {
...
// RT            rowModelMapper = new SizeSequenceMapper(filters);
            rowModelMapper = new SizeSequenceMapper();		// RT
        }
