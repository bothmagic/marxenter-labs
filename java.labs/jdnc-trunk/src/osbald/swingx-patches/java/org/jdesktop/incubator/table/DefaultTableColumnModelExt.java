/*
 * $Id: DefaultTableColumnModelExt.java 3012 2009-02-04 15:13:47Z osbald $
 *
...
//----------------------- implement TableColumnModelExt
    ...
    /**
     * @return Returns all columns, hidden and visible in their current view order.
     */
    public List<TableColumn> getOrderedColumns() {
        return Collections.unmodifiableList(currentColumns);
    }

    /**
     * Similar to getColumnIndex() but queries both hidden and visible columns.
     *
     * @param identifier the identifier object
     * @return the view index of the first column whose identifier is equal to <code>identifier</code>
     ...
     * @see #getColumnIndex
     */
    public int getOrderedColumnIndex(Object identifier) {
        if (identifier == null) {
            throw new IllegalArgumentException("Identifier is null");
        }
        List<TableColumn> tableColumns = getOrderedColumns();
        for (int i = tableColumns.size() - 1; i >= 0; i--) {
            if (identifier.equals(tableColumns.get(i).getIdentifier())) {
                return i;
            }
        }
