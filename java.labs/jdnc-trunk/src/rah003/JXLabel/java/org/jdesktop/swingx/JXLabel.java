/*
 * $Id: JXLabel.java 1375 2007-06-05 06:30:43Z rah003 $
 *
...
public class JXLabel extends JLabel {
    ...
    protected void paintComponent(Graphics g) {
        if (ignoreRepaint) {
            // System.out.println("paint recur");
            return;
        }
        if (backgroundPainter == null && foregroundPainter == null) {
            ...
            if (foregroundPainter != null) {
                ...
                if ((textRotation > 4.697 && textRotation < 4.727) || (textRotation > 1.555 && textRotation < 1.585)) {
                    ...
                    // free angle - just fit square inside (until someone comes up with something better)

                    ignoreRepaint = true;
                    // System.out.println("Starting: w" + getWidth() + ", h:" + getHeight());
                    double square = Math.min(getHeight(), getWidth()) * Math.cos(Math.PI / 4d);

                    View v = (View) getClientProperty(BasicHTML.propertyKey);
                    if (v == null) {
                        // crap. no html and no wrapline enabled means no view
                        // ... find another way to figure out the heigh & width
                        ...
                        // System.out.println("tcw:" + cw + ", tch:" + ch);
                        // min of whichever is above 0 (!!! no min of abs values)
                        tx = cw < 0 ? ch : ch > 0 ? Math.min(cw, ch) : cw;
                    } else {

                        // System.out.println("H:" + BasicHTML.isHTMLString(getText()));
                        // System.out.println("X:" + v.getPreferredSpan(View.X_AXIS));
                        // System.out.println("Y:" + v.getPreferredSpan(View.Y_AXIS));
                        ...
                        while (!ready) {
                            ...
                            while (h == v.getPreferredSpan(View.Y_AXIS)) {
                                w -= 10;
                                v.setSize(w, h);
                                // System.out.println("aX:" + v.getPreferredSpan(View.X_AXIS) + ", aY:"
                                // + v.getPreferredSpan(View.Y_AXIS));
                            }
                            if (w < square || h > square) {
                                ...
                                // System.out.println("'s not gonna fit. bail out. sq:" + square);
                                w = h = (float) square;
                            ...
                            // System.out.println("sina:" +Math.sin(alpha) + "cosa:" + Math.cos(alpha));
                            ...
                            c--;
                            // c = - cw;
                            // System.out.println("setting w to " + c);
                            if (c > w) {
                                // System.out.println("done");
                                v.setSize((float) c, 10 * h);
                                ready = true;
                            } else {
                                // w = v.getPreferredSpan(View.X_AXIS);
                                v.setSize((float) c, 10 * h);
                                if (v.getPreferredSpan(View.Y_AXIS) > h) {
                                    // System.out.println("need more rounds. h up at " +
                                    // v.getPreferredSpan(View.Y_AXIS));
                                    // set size back to figure out new line break and height after
                                    v.setSize(w, 10 * h);
                                } else {
                                    // System.out.println("done at c:" + c + ", w:" + w);
                                    w = (float) c;
                                    ready = true;
                                }
                            }
                        ...
                        ty = h;// v.getPreferredSpan(View.Y_AXIS);
                    }
                    pWidth = (int) tx;
                    pHeight = (int) ty;
                    ignoreRepaint = false;
                ...
                double wx1 = Math.sin(textRotation) * ty;
                double wx2 = Math.cos(textRotation) * tx;
                // System.out.println("wx1:" + wx1 + ", wx2:" + wx2);
                double wx = wx1 + wx2;
                ...
                // System.out.println("x:" + x + ", y:" + y + ", wx:" + wx + ", wy:" + wy + ", w:" + pWidth + ", h:"
                // + pHeight + ", ow:" + getWidth() + ", oh:" + getHeight());
                g2.translate(x, y);
