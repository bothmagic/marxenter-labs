...
/*     setText((value == null) ? "---" : formatter.format(value));  */
 ...
 public Component getTableCellRendererComponent(JTable table,
	...
	*/
        ...
        System.out.println(" ren compo = " + renderer.getClass().getName());
        
        
        
           if(value != null && value.toString().trim().length() > 0) {
                  ((IndicatorRenderer)renderer).setIcon(textIcon);
                  
                  if(value.toString().indexOf("-") > -1) {
                      if(currentForeground == null)
                            currentForeground = renderer.getForeground();
                      
                      ((IndicatorRenderer)renderer).setForeground(Color.GREEN);
                  }
                  else
                      ((IndicatorRenderer)renderer).setForeground(currentForeground);
                  
           } else {
                  ((IndicatorRenderer)renderer).setIcon(null);
           }
