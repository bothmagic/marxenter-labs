...
class XPStyle {
     ...
     *    in the current style
     */
    synchronized Color getColor(Skin skin, TMSchema.Prop prop, Color fallback) {
	...
	TMSchema.Part part = skin.part;
        ...
        if (color == null) {
            color = ThemeReader.getColor(part.getControlName(null), part.getValue(),
                                         TMSchema.State.getValue(part, skin.state),
					 prop.getValue());
            if (color != null) {
    ...
    synchronized Border getBorder(Component c, TMSchema.Part part) {
        if (part == TMSchema.Part.MENU) {
            ...
            if (flatMenus) {
                // To Do: The classic border uses this color, but we should
    ...
    private class XPStatefulFillBorder extends XPFillBorder {
        ...
        public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
	        ...
	        /*   removed this, since it is specifically for a combo box
    ...
    private class XPImageBorder extends AbstractBorder implements UIResource {
        ...
        XPImageBorder(Component c, TMSchema.Part part) {
     ...
     */
    synchronized Skin getSkin(Component c, TMSchema.Part part) {
        assert isSkinDefined(c, part) : "part " + part + " is not defined";
        return new Skin(c, part, null);
    }


    long getThemeTransitionDuration(Component c, TMSchema.Part part, TMSchema.State stateFrom,
                                    TMSchema.State stateTo, TMSchema.Prop prop) {
         return ThemeReader.getThemeTransitionDuration(part.getControlName(c),
                                          part.getValue(),
                                          TMSchema.State.getValue(part, stateFrom),
                                          TMSchema.State.getValue(part, stateTo),
	...
	Skin(Component component, TMSchema.Part part, TMSchema.State state) {
        ...
        Insets getContentMargin() {
            ...
            return ThemeReader.getThemeMargins(part.getControlName(null), part.getValue(),
                                               0, TMSchema.Prop.SIZINGMARGINS.getValue());
        }
