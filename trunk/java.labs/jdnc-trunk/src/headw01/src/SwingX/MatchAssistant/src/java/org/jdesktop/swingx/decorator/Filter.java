/*
 * $Id: Filter.java 2278 2008-02-19 17:51:16Z headw01 $
 *
...
//---------------------------------- for subclasses
    ...
    /**
     * Tests for a match of the cell at the specified row and column (in model coordinates).
     *
     * @param row in the coordinates of what is the filter's "view" of the model
     * @param column in model coordinates
     * @return SearchResult if the Pattern matches the value at the given cell, <code>null</code> otherwise.
     */
    protected SearchResult findMatchAt(Pattern pattern, int row, int column, boolean mapToModel) {
        if (mapToModel) {
            row = mapTowardModel(row);
        }
        Filter filter = getMappingFilter();
        if (filter != null) {
            // The mapping filter is not the currently executing filter 
            // (since we are down the chain, and are calling it).  So, we want that filter to
            // map the row we are passing toward the model.
            return filter.findMatchAt(pattern, row, column, true);
        }
        if (adapter != null) {
            // Suppress unchecked warning because we know we are checking the component from the 
            // ComponentAdapter that we are getting the MatchAssistant from.  It has to match.
            @SuppressWarnings("unchecked")
            MatchAssistant<JComponent> assistant = adapter.getMatchAssistant();
            // convertViewToModel is false here because we have already mapped our way up the FilterPipeline.
            return assistant.findMatchAt(adapter.getComponent(), pattern, row, column, false);
        }
