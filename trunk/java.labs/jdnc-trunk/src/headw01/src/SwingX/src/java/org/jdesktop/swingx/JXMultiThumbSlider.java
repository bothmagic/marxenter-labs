/*
 * $Id: JXMultiThumbSlider.java 2103 2008-01-09 17:40:22Z headw01 $
 *
...
public class JXMultiThumbSlider<E> extends JComponent {
   ...
   /**
     ...
     * <code>Slider</code> is editable or not.
     *
     ...
     * <code>Slider</code> should be editable.
    ...
    private void initializeThumbs() {
        ...
        java.util.List<Thumb<E>> tl = model.getSortedThumbs();

        int controlHeight = 16;
        for(int i = 0; i < tl.size(); i++) {
            Thumb<E> t = tl.get(i);
            ThumbComp thumb = new ThumbComp(JXMultiThumbSlider.this);
            thumb.setLocation(0, controlHeight - thumb.getSize().height - 3);
            add(thumb);
            thumbs.add(i, thumb);
            clipThumbPosition(thumb);
            setThumbXByPosition(thumb, t.getPosition());
        }
    ...
    private class MultiThumbMouseListener extends MouseInputAdapter {
        ...
        public void mouseDragged(MouseEvent evt) {
            ...
            if(selected != null) {
                ...
                if(nx > getWidth()-selected.getWidth()/2) {
                    nx = getWidth()-selected.getWidth()/2;
                }
    ...
    private static class ThumbComp extends JComponent {
        ...
        public ThumbComp(JXMultiThumbSlider<?> slider) {
            this.slider = slider;
            Dimension dim = new Dimension(10,Math.max(10, slider.getHeight()));
    ...
    private class ThumbHandler implements ThumbDataListener {
        ...
        public void thumbAdded(ThumbDataEvent evt) {
            ThumbComp thumb = new ThumbComp(JXMultiThumbSlider.this);
            int controlHeight = 16;
            thumb.setLocation(0, controlHeight - thumb.getSize().height);
            add(thumb);
