/*
 * $Id: AbstractAreaPainterX.java 1193 2007-04-03 16:20:57Z kleopatra $
 *
 ...
 */
public abstract class AbstractAreaPainterX<T> extends AbstractLayoutPainterX<T> {
    ...
    private PropertyChangeListener effectListener;
    
    /**
     * Creates a new instance of AbstractAreaPainter
     */
    public AbstractAreaPainterX() {
        fillPaint = Color.RED;
    }
    /**
     * Creates a new instance of AbstractAreaPainter
     ...
     */
    public AbstractAreaPainterX(Paint paint) {
    ...
    public void setAreaEffects(AreaEffect... areaEffects) {
        ...
        installEffectListener(getAreaEffects());
        clearCache();
        firePropertyChange("areaEffects", old, getAreaEffects());
    }

    private void installEffectListener(AreaEffect[] old) {
        for (int i = 0; i < old.length; i++) {
            if (old[i] instanceof AbstractAreaEffect) {
                ((AbstractAreaEffect) old[i]).addPropertyChangeListener(getEffectListener());
            }
        }
    }

    private void uninstallEffectListener(AreaEffect[] old) {
        for (int i = 0; i < old.length; i++) {
            if (old[i] instanceof AbstractAreaEffect) {
                ((AbstractAreaEffect) old[i]).removePropertyChangeListener(getEffectListener());
            }
        }
     ...
     */
    protected PropertyChangeListener getEffectListener() {
        if (effectListener == null) {
            effectListener = createPainterListener();
        }
