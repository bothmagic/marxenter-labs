/*
 * $Id: TreeTableCellEditorX.java 790 2006-04-03 11:44:48Z kleopatra $
 *
 ...
 * to do something similiar. <p>
 *
 * This is similar in mis-function to the current head (as Shai 
 * tried to fix it) A big problem is correctly position the 
 * textfield in ltor. Another weirdness (even in RToL) is that
 * the column is not reliably updated after an edit.
 * 
 *
 * @author Scott Violet
 * @author Ramesh Gupta
 */
public class TreeTableCellEditorX extends DefaultCellEditor {
    private static final Logger LOG = Logger
            .getLogger(TreeTableCellEditorX.class.getName());
    
    public TreeTableCellEditorX(JTree tree) {
        super(new TreeTableTextField());
//        super(new TreeTableTextFieldOld());
        if (tree == null) {
...
//        int iconOffset = getIconOffset(row, isSelected);
//        ((TreeTableTextFieldOld) getComponent()).iconOffset = iconOffset;
//        Rectangle rowBounds = null;
//        if (tree != null) {
//            rowBounds = tree.getRowBounds(row);
//        }
//        ((TreeTableTextFieldOld) getComponent()).rowBounds = rowBounds;
        return component;
    }
...
//            offset += icon.getIconWidth() + label.getIconTextGap();
     ...
     * @return
     */
    protected int getIconOffset(int row, boolean isSelected) {
        if (tree == null) return 0;
        Rectangle bounds = tree.getRowBounds(row);
        LOG.info("rowbounds - " + bounds);
//        int offset = bounds.x;
        Object node = tree.getPathForRow(row).getLastPathComponent();
        boolean leaf = tree.getModel().isLeaf(node);
        boolean expanded = tree.isExpanded(row);
     ...
     * make the x location be <code>offset</code>.
     */
    static class TreeTableTextFieldOld extends JTextField {
        int iconOffset; // changed to package private instead of public
        Rectangle rowBounds;
        
        public void reshape(int x, int y, int width, int height) {
            LOG.info("reshape - " + new Rectangle(x, y, width, height));
            // Allows precise positioning of text field in the tree cell.
            //Border border = this.getBorder(); // get this text field's border
            //Insets insets = border == null ? null : border.getBorderInsets(this);
            //int newOffset = offset - (insets == null ? 0 : insets.left);
            if (getComponentOrientation().isLeftToRight()) {
                int newOffset = rowBounds.x + iconOffset - getInsets().left;
                super.reshape(x + newOffset, y, width - newOffset, height);
            } else {
                int newWidth = iconOffset + getInsets().right; // + rowBounds.x;
                super.reshape(x, y, width - newWidth, height);
            }
        }
    }
