/*
 * $Id: JXXTreeTableIssues.java 3205 2009-08-31 13:18:21Z kleopatra $
 * 
...
import org.jdesktop.swingx.JXTreeTableVisualCheck.SimpleTestModel;
...
/**
 * Test to exposed known issues of <code>JXXTreeTable</code>. <p>
 * 
 ...
 */
public class JXXTreeTableIssues extends InteractiveTestCase {
    private static final Logger LOG = Logger.getLogger(JXXTreeTableIssues.class
            .getName());
    public static void main(String[] args) {
        setSystemLF(true);
        JXXTreeTableIssues test = new JXXTreeTableIssues();
        try {
//            test.runInteractiveTests();
//            test.runInteractiveTests(".*AdapterDeleteUpdate.*");
//            test.runInteractiveTests(".*Clip.*");
//            test.runInteractiveTests(".*TreeExpand.*");
    ...
    public void interactiveTreeTableWinPerformance() {
        JXXTreeTable table = new JXXTreeTable(new SimpleTestModel());
    ...
    public void interactiveXRendererCustomColor() {
        ...
        DefaultTableRenderer swingx = new DefaultTableRenderer();
        ...
        treeTable.getColumn(0).setCellRenderer(swingx);
...
//        other.setBackgroundSelectionColor(Color.RED);
     ...
     *  - start edit a cell in the hierarchical column, - click into another
     * cell of the hierarchical column - edit sometimes canceled instead of
     * stopped
     * 
    ...
    // JXXTreeTable xTable = new JXXTreeTable(new ComponentTreeTableModel(new
    // JXFrame()));
    // xTable.expandAll();
    ...
    // JXFrame frame = wrapWithScrollingInFrame(xTable, "#730-swingx: click
    // sometimes cancels");
    ...
    /**
     * Issue #493-swingx: incorrect table events fired. Issue #592-swingx: (no
     * structureChanged table events) is a special case of the former.
     * 
     * Here: add support to prevent a structureChanged even when setting the
     * root. May be required if the columns are stable and the model lazily
     * loaded. Quick hack would be to add a clientProperty?
     * 
     */
    public void testTableEventOnSetRootNoStructureChange() {
        TreeTableModel model = createCustomTreeTableModelFromDefault();
        final JXXTreeTable table = new JXXTreeTable(model);
        table.setRootVisible(true);
        table.expandAll();
        final TableModelReport report = new TableModelReport(table.getModel());
        ((DefaultTreeTableModel) model)
                .setRoot(new DefaultMutableTreeTableNode("other"));
        assertEquals("tableModel must have fired", 1, report.getEventCount());
        assertTrue("event type must be dataChanged "
                + TableModelReport.printEvent(report.getLastEvent()), report
                .isDataChanged(report.getLastEvent()));
    ...
    public void interactiveScrollAlternateHighlightDefaultModel() {
        ...
        final JXXTreeTable treeTable = new JXXTreeTable(new DefaultTreeTableModel(root));
        treeTable.expandAll();
    ...
    /**
     * Issue #493-swingx: JXXTreeTable.TreeTableModelAdapter: Inconsistency
    ...
    public void interactiveTreeTableModelAdapterDeleteUpdate() {
        ...
        final JXXTreeTable table = new JXXTreeTable(customTreeTableModel);
        table.setRootVisible(true);
        table.expandAll();
        ...
        final JXFrame frame = wrapWithScrollingInFrame(table, xtree,
                "JXXTreeTable.TreeTableModelAdapter: Inconsistency firing update on recursive delete");
...
//                    }     
    ...
    public void interactiveTreeTableModelAdapterUpdate() {
        ...
        xtree.expandAll();
        final JXFrame frame = wrapWithScrollingInFrame(table, xtree,
                "JXXTreeTable.TreeTableModelAdapter: Inconsistency firing update");
    ...
    public void interactiveTreeTableModelAdapterMutateSelected() {
        ...
        xtree.expandAll();
        final JXFrame frame = wrapWithScrollingInFrame(table, xtree,
                "JXXTreeTable.TreeTableModelAdapter: Inconsistency firing delete expanded folder");
    ...
    /**
     * Issue #??-swingx: hyperlink in JXXTreeTable hierarchical column not
     * active.
     * 
     */
    public void interactiveTreeTableLinkRendererSimpleText() {
        ...
        JXXTreeTable tree = new JXXTreeTable(new FileSystemModel());
        ...
        tree.getColumn(0).setCellRenderer(new DefaultTableRenderer(provider));
...
//        tree.setCellRenderer(new LinkRenderer(simpleAction));
    ...
    public void interactiveTreeTableCustomRenderer() {
        ...
        ComponentProvider<?> provider = new CheckBoxProvider(sv, JLabel.LEADING);
//            /**
...
//        };
    ...
    public void interactiveTextAreaTreeTable() {
        ...
        JXXTreeTable treeTable = new JXXTreeTable(model);
        ...
        treeTable.getColumn(0).setCellRenderer(renderer);
    ...
    public void interactiveTreeTableClipIssueWrappingProvider() {
        final JXXTreeTable treeTable = new JXXTreeTable(createActionTreeModel());
...
//              rendererComponent.setBorder(redBorder);
        ...
        DefaultTableRenderer treeCellRenderer = new DefaultTableRenderer();
//                wrappingProvider);
        treeTable.getColumn(0).setCellRenderer(treeCellRenderer);
        ...
        tree.setCellRenderer(new DefaultTreeRenderer(wrappingProvider));
    ...
    public void interactiveTreeTableClipIssueDefaultRenderer() {
        ...
        JXFrame frame = wrapWithScrollingInFrame(treeTable, 
                tree, "JXXTreeTable vs. JTree: default renderer");
        Action revalidate = new AbstractActionExt("revalidate") {
     ...
     * treeTableModel.getValueAt(...) for showing. Outdated?
     * 
     */
//    public void interactiveTreeTableGetValueRenderer() {
//        JXXTreeTable tree = new JXXTreeTable(new ComponentTreeTableModel(new JXFrame()));
...
//          
//        };
//        tree.setTreeCellRenderer(new DefaultTreeTableRenderer(provider));
//        tree.expandAll();
//        tree.setHighlighters(HighlighterFactory.createSimpleStriping());
...
//------------- unit tests    
    ...
    /**
     * TreeSelectinModel must be same as layoutCache.
     */
    public void testSelectionModel() {
        JXXTreeTable treeTable = new JXXTreeTable();
        assertEquals(treeTable.getTreeSelectionModel(), 
                ((XOutlineModel) treeTable.getModel()).getLayout().getSelectionModel());
    }
    
    /**
     * Extended to widen access to some properties for testing.
     */
    public static class XBasicTreeUI extends BasicTreeUI {
    ...
    /**
     * test setting the large model property on JTree.
     * UI doesn't if the rowHeight is < 1.
     */
    public void testLargeModelCompareJTree() {
        JTree outline = new JTree();
        outline.setUI(new XBasicTreeUI());
        assertEquals(0, outline.getRowHeight());
        outline.setLargeModel(true);
        // at this point the ui's largeModel property is different from the
        // tree's equivalent. The cache had not been changed! But how to test:
        // the ui's methods are protected ...
        assertEquals(outline.isLargeModel(), ((XBasicTreeUI) outline.getUI()).isLargeModel());
    }
    ...
    public void testRowHeightCompareJTree() {
        ...
        // the ui's methods are protected ...
        assertEquals(outline.isLargeModel(), ((XBasicTreeUI) outline.getUI()).isLargeModel());
        // reset rowheight to invalid for largeModel
        outline.setRowHeight(0);
        assertEquals(0, outline.getRowHeight());
        // at this point the ui's largeModel property again is different from the
        assertEquals(outline.isLargeModel(), ((XBasicTreeUI) outline.getUI()).isLargeModel());
    }
     ...
     * JXTreeTable adapter had issues with not guarding against null node: 
     * strictly, it's invalid to access with such, but there where contexts 
     * from core when it happened. Need to keep here until we know for sure
     * it was caused by JTree/UI and not by any of the exatracted pieces.
     */
    public void testConservativeAccess() {
        TreeTableModel normal = createCustomTreeTableModelFromDefault();
        // for testing we need a model which relies on 
        // node != null
        TreeTableModel model = new DefaultTreeTableModel((TreeTableNode) normal.getRoot()) {
            @Override
            public int getColumnCount() {
                return 1;
            }
        ...
        XOutlineModel outline =  new DefaultXOutlineModel(model);
        outline.collapseRow(0);
        assertEquals(1, outline.getRowCount());
        // simulate contexts where the accessed row isn't currently visible
        outline.getValueAt(outline.getRowCount(), 0);
        outline.isCellEditable(outline.getRowCount(), 0);
        outline.setValueAt("somename", outline.getRowCount(), 0);
    ...
    public void testSelectionKeepsEditingWithExpandsTrue() {
        ...
        assertEquals("after selection treeTable editing state must be unchanged", 
                canEdit, treeTable.isEditing());
    }
    ...
    public void testLeadSelectionFromTable() {
        JXXTreeTable treeTable = prepareTreeTable(false);
    ...
    public void testLeadAfterRemoveSelectionFromTree() {
        JXXTreeTable treeTable = prepareTreeTable(true);
     ...
     * @return
     */
    protected JXXTreeTable prepareTreeTable(boolean selectFirstRow) {
        JXXTreeTable treeTable = new JXXTreeTable(new ComponentTreeTableModel(new JXFrame()));
        treeTable.setRootVisible(true);
