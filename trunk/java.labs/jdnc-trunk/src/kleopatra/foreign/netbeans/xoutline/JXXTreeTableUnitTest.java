/*
 * $Id: JXXTreeTableUnitTest.java 3029 2009-03-18 11:01:06Z kleopatra $
 * 
...
public class JXXTreeTableUnitTest extends InteractiveTestCase {
    @SuppressWarnings ("unused")
    private static final Logger LOG = Logger
            .getLogger(JXXTreeTableUnitTest.class.getName());
    
    protected TreeTableModel treeTableModel;
    protected TreeTableModel simpleTreeTableModel;
    
    public JXXTreeTableUnitTest() {
        super("JXXTreeTable Unit Test");
    }

    
    public void testExpandsSelectedCompareJTree() {
        JTree treeTable = new JTree(createCustomTreeTableModelFromDefault());
        treeTable.setRootVisible(true);
        assertEquals(4, treeTable.getRowCount());
        TreePath childPath = treeTable.getPathForRow(1);
        treeTable.collapseRow(0);
        assertFalse("sanity: child hidden", treeTable.isVisible(childPath));
        treeTable.getSelectionModel().setSelectionPath(childPath);
        assertTrue("child must be visible after selection", treeTable.isVisible(childPath));
    }
    
    /**
     * test expandsSelectedPath property. 
     */
    public void testExpandsSelected() {
        JXXTreeTable treeTable = new JXXTreeTable(createCustomTreeTableModelFromDefault());
        treeTable.setRootVisible(true);
        assertEquals(4, treeTable.getRowCount());
        TreePath childPath = treeTable.getPathForRow(1);
        treeTable.collapseRow(0);
        assertFalse("sanity: child hidden", treeTable.isVisible(childPath));
        treeTable.getTreeSelectionModel().setSelectionPath(childPath);
        assertTrue("child must be visible after selection", treeTable.isVisible(childPath));
    }


    public void testExpandsSelectedProperty() {
        JXXTreeTable treeTable = new JXXTreeTable();
        boolean initial = true;
        assertEquals(initial, treeTable.getExpandsSelectedPaths());
        PropertyChangeReport report = new PropertyChangeReport();
        treeTable.addPropertyChangeListener(report);
        treeTable.setExpandsSelectedPaths(!initial);
        TestUtils.assertPropertyChangeEvent(report, "expandsSelectedPaths", initial, !initial);
    }
    
    public void testScrollsOnExpandProperty() {
        JXXTreeTable treeTable = new JXXTreeTable();
        boolean initial = false;
        assertEquals(initial, treeTable.getScrollsOnExpand());
        PropertyChangeReport report = new PropertyChangeReport();
        treeTable.addPropertyChangeListener(report);
        treeTable.setScrollsOnExpand(!initial);
        TestUtils.assertPropertyChangeEvent(report, "scrollsOnExpand", initial, !initial);
    ...
    public void testTableEventUpdateOnTreeTableSetValueForRoot() {
        ...
        assertTrue("root must be editable", table.getModel().isCellEditable(0,
                0));
        ...
        assertEquals("the event type must be update "
                + TableModelReport.printEvent(report.getLastEvent()), 1, report
                .getUpdateEventCount());
     ...
     *
     */
    public void testToggleLargeModel() {
        JXXTreeTable treeTable = new JXXTreeTable();
    ...
    public void testDisabledTreeColumn() {
        ...
        TableCellRenderer renderer = treeTable.getCellRenderer(0, 0);
        assertEquals(false, (treeTable.prepareRenderer(renderer, 0, 0).isEnabled()));
    }
    ...
    /**
     * Issue #821-swingx: JXXTreeTable broken string rep of hierarchical column
     * Here we test the getStringAt for the hierarchical column.
     */
    public void testGetStringAtHierarchicalColumn() {
        JXXTreeTable table = new JXXTreeTableT(AncientSwingTeam.createNamedColorTreeTableModel());
     ...
     * 
     */
    public static class JXXTreeTableT extends JXXTreeTable {

        public JXXTreeTableT(TreeTableModel model) {
            super(model);
        }
...
//    }
    ...
    public void testSelectionMapperViewSelectionModel() {
        JXXTreeTable one = new JXXTreeTable();
    ...
    public void testSelectionMapper() {
        JXXTreeTable one = new JXXTreeTable();
        JXXTreeTable two = new JXXTreeTable();
...
//    }
    ...
    public void testTableEventUpdateOnTreeTableModelSetValue() {
        ...
        model.setValueAt("games",
                table.getPathForRow(row).getLastPathComponent(), 0);
        assertEquals("table value must be new", "games", table.getValueAt(row, 0));
        assertEquals("tableModel must have fired", 1, report.getEventCount());
...
//        JXXTreeTable treeTable = new JXXTreeTable(new ComponentTreeTableModel(new JXErrorPane()));
...
//    }
    ...
    public void testComponentAdapterIsLeaf() {
        ...
        JXXTreeTableT table = new JXXTreeTableT(model);
        table.expandAll();
        // sanity
    ...
    /**
     * Issue #342-swingx: default margins in JXXTreeTable.
     * 
    ...
    public void testShowGrid() {
        JXXTreeTable table = new JXXTreeTable(simpleTreeTableModel);
...
//    }
    ...
    protected JXXTreeTable prepareTreeTable(boolean selectFirstRow) {
        JXXTreeTable treeTable = new JXXTreeTable(simpleTreeTableModel);
        treeTable.setRootVisible(true);
    ...
    /**
     * Issue #230-swingx: 
     * JXXTreeTable should fire property change on setTreeTableModel.
     * 
     * 
     *
     */
    public void testTreeTableModelIsBoundProperty() {
        JXXTreeTable treeTable = new JXXTreeTable();
        TreeTableModel oldValue = treeTable.getTreeTableModel();
        PropertyChangeReport report = new PropertyChangeReport();
        treeTable.addPropertyChangeListener(report);
        treeTable.setTreeTableModel(simpleTreeTableModel);
        TestUtils.assertPropertyChangeEvent(report, "treeTableModel", oldValue, simpleTreeTableModel);
        // sanity: must not fire when setting to same
        report.clear();
        treeTable.setTreeTableModel(simpleTreeTableModel);
        assertEquals("treeTable must not have fired", 0, report.getEventCount("treeTableModel")); 
    }
