...
public class CellContext implements Serializable {
    ...
    private static final Logger LOG = Logger.getLogger(CellContext.class
            .getName());
     ...
     * the border is LF specific.<p>
     * 
     * @return the focus border of the rendered component.
     */
    protected Border getFocusBorder() {
        Border border = getUIFocusBorder();
        if (border != null && getUINoFocusBorder() != null) {
            return border;
        }
        Border fontBased = getFontBasedBorder();
        return border != null ? BorderFactory.createCompoundBorder(border, fontBased) : fontBased;
    }

    /**
     * @return
     */
    protected Border getUIFocusBorder() {
        Border border = null;
        if (isSelected()) {
            border = UIManager
     ...
     * <p>
     * 
     * Here: the borders are adjusted to the component's fontSize if the LAF does
     * <b>not</b> provide a noFocusCellBorder. If it does, returns the no/focusBorder
     * as is. 
     * 
    ...
    protected Border getBorder() {
        ...
        Border border = getUINoFocusBorder();
        if (border != null) {
            return border;
        }
        Insets fontBasedInsets = getFontBasedInsets();
        if (fontBasedInsets != null) {
            Border focusBorder = getUIFocusBorder();
            if (focusBorder != null) {
                fontBasedInsets = SizeUtils.addInsets(fontBasedInsets, focusBorder.getBorderInsets(null));
            }
            return BorderFactory.createEmptyBorder(fontBasedInsets.top, 
                    fontBasedInsets.left, fontBasedInsets.bottom, fontBasedInsets.right);
        }
     ...
     * @return
     */
    private Insets getFontBasedInsets() {
        Font font = getFont();
        if (font != null) {
            return SizeUtils.getListCellRendererInsets(font.getSize());
        }
        return null;
    }

    protected Border getUINoFocusBorder() {
        return UIManager.getBorder(getUIKey("cellNoFocusBorder"));
    }
    
    /**
     * 
     */
    protected Border getFontBasedBorder() {
        Font font = getFont();
        if ((font != null)){
            Insets insets = SizeUtils.getListCellRendererInsets(font.getSize());
    ...
    protected Font getFont() {
        Font font = getComponent() != null ? getComponent().getFont() : null;
        if (font == null) {
            font = UIManager.getFont(getUIKey("font"));
        }
        return font;
    }
