/*
 * $Id: AbstractActionExt.java 517 2005-05-29 08:23:31Z vprise $
 *
...
import org.jdesktop.jdnc.incubator.vprise.UserPermissions;
...
public abstract class AbstractActionExt extends AbstractAction
    ...
    /**
     * These are the user roles that are allowed to make use of this action. If an
     * allowed role is detected then the action will be enabled.
     */
    private Object[] allowedRoles = null;
    ...
    public AbstractActionExt(AbstractActionExt action) {
        ...
        UserPermissions.getInstance().addPropertyChangeListener("roles", new PropertyChangeListener() {
     ...
     * allowed role is detected then the action will be enabled.
     * 
     * An example for an allowed set of roles would be:
     * new Object[] { "Manager", new String[] { "SeniorEmployee", "FinanceDept"}, "CEO" }
     * Which would mean that the Manager and CEO can use this operation, a senior emplyee
     * can use this action only if he is from the finance department.
     *
     * @param allowedRoles array of Strings and String arrays containing user roles or null
     * to disable access control
     */
    public void setAllowedRoles(Object[] allowedRoles) {
        this.allowedRoles = allowedRoles;
        updateRoles();
    }

    /**
     * Invoked to update the roles enabled/disabled state
     */
    private void updateRoles() {
        ...
        for(int iter = 0 ; iter < allowedRoles.length ; iter++) {
            if(allowedRoles[iter] instanceof String) {
                if(UserPermissions.getInstance().isUserInRole((String)allowedRoles[iter])) {
                    setEnabled(true);
                    return;
                }
            } else {
                if(UserPermissions.getInstance().isUserInRoles((String[])allowedRoles[iter])) {
                    setEnabled(true);
                    return;
                }
            }
