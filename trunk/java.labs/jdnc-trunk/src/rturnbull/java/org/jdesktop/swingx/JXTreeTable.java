...
import org.jdesktop.swingx.tree.TreeTableSorter;
...
public class JXTreeTable extends JXTable {
    ...
    // RT
    /**
     * sorter is TreeTableSorter
     */
    private RowSorter<? extends TableModel> sorter = null;
...
//        setRootVisible(rootVisible);
        // RT add
    	if (treeModel != null && treeModel instanceof AbstractTreeTableModel 
    			&& jxAutoCreateRowSorter) {
    		setRowSorter(new TreeTableSorter<TableModel>(getModel(), 
...
//------------------------------ exposed tree listeners
    ...
    public void setAutoCreateRowSorter(boolean autoCreateRowSorter) {
    	...
    	if (m instanceof AbstractTreeTableModel) {
			boolean oldValue = jxAutoCreateRowSorter;
			jxAutoCreateRowSorter = autoCreateRowSorter;
			AbstractTreeTableModel model = (AbstractTreeTableModel) m;
			if (model != null && autoCreateRowSorter) {
				setRowSorter(new TreeTableSorter<TableModel>(getModel(), model));
			}
    ...
    public void setRowSorter(RowSorter<? extends TableModel> sorter) {
    	if (!(sorter instanceof TreeTableSorter)) {
    		throw new IllegalArgumentException(
    				"Sorter must be a TreeTableSorter");
    	}
    	TreeTableSorter newSorter = (TreeTableSorter) sorter;
    	if (this.sorter != null && this.sorter instanceof TreeTableSorter) {
    		removeTreeExpansionListener((TreeTableSorter)this.sorter);
    	}
    	this.sorter = sorter;
    	addTreeExpansionListener(newSorter);
    ...
    public void toggleSortOrder(int column) {
    	...
    	if (sorter instanceof TreeTableSorter) {
    		((TreeTableSorter<? extends TableModel>)sorter).toggleSort(column);    		
    	} else {
	...
	public void removeSort(int column) {
        if (sorter == null) return;
        if (!(sorter instanceof TreeTableSorter)) return;
        column = convertColumnIndexToModel(column);
        ((TreeTableSorter<? extends TableModel>)sorter).removeSort(column);
    }
    
	private void updateSorterFromColumns() {
    	TableColumnModelExt model;
    	TreeTableSorter<? extends TableModel> s; 
    	try {
			model = (TableColumnModelExt)getColumnModel();
			s = (TreeTableSorter<? extends TableModel>)sorter;
		...
		for (int i = 0; i < cols.size(); i++) {
			...
			if (column.getComparator() != null) {
				s.setComparator(i, column.getComparator());
			}
			s.setSortable(i, column.isSortable());
		}
    }
