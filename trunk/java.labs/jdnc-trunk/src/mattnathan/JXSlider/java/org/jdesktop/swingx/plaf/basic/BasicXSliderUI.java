/*
 * $Id: BasicXSliderUI.java 2766 2008-10-09 15:32:38Z mattnathan $
 *
...
import org.jdesktop.swingx.slider.DefaultSliderValueRenderer;
import org.jdesktop.swingx.slider.IconSliderTrackRenderer;
import org.jdesktop.swingx.slider.MarkerGroup;
import org.jdesktop.swingx.slider.MarkerMutator;
import org.jdesktop.swingx.slider.MarkerRange;
import org.jdesktop.swingx.slider.MarkerSelectionModel;
import org.jdesktop.swingx.slider.Projection;
import org.jdesktop.swingx.slider.SliderTrackRenderer;
import org.jdesktop.swingx.slider.SliderValueRenderer;
import org.jdesktop.swingx.slider.ViewRange;
...
public class BasicXSliderUI extends XSliderUI implements LazyActionMap.ActionMapLoader {
    ...
    protected void paintMarkers(Graphics g, JXSlider slider, MarkerGroup group, int i,
        ...
        for (int j = 0, l = markers.getSize(); j < l; j++) {
            ...
            if (selectedValue != null && val == selectedValue) {
                ...
                Component comp = r.getSliderValueRendererComponent(slider, val, i, group, j, markers,
                      focused && selected && selectedValue != null && Long.valueOf(val).equals(selectedValue), slider.isEnabled(), false);
        ...
        if (selIndex >= 0) {
            long tmpVal = selectedValue == null ? 0 : selectedValue;
            Component comp = r.getSliderValueRendererComponent(slider,
                  tmpVal, i, group, selIndex, markers,
            ...
            int cy;
            if (horizontal) {
                cx = bounds.x + (int) (bounds.width * ((tmpVal - min) / (double) (max - min))) - (int) (pref.width * ax);
                cy = bounds.y + baseline - (int) (pref.height * ay);
            } else {
                cx = bounds.x + baseline - (int) (pref.width * ax);
                cy = bounds.y + (int) (bounds.height * ((tmpVal - min) / (double) (max - min))) - (int) (pref.height * ay);
            }
            int cw = pref.width;
            int ch = pref.height;
     ...
     * int should represent the baseline or anchor point along the sliders orientation axis for values to be rendered
     * relative to.
     *
     * @param g The graphics to paint to.
    ...
    /**
     * Moves the active value either positively of negatively by one block unit.
     *
     ...
     *
     * @param s The slider to adjust.
     * @param positive {@code true} if the value should have increased.
     * @param valueAdjusting Whether this is just a point in a series of changes.
     */
    ...
    public long locationToValue(JXSlider slider, int x, int y) {
        ...
        switch (slider.getOrientation()) {
            case HORIZONTAL:
                result = vmin + Math.round((((long) x - (long) cb.x) / (double) cb.width) * (vmax - vmin));
                break;
            case VERTICAL:
                result = vmin + Math.round((((long) y - (long) cb.y) / (double) cb.height) * (vmax - vmin));
                break;
        }
        return result;
    ...
    /**
     * Converts the given value into a point on the slider. If the given value is not visible then null is returned.
     *
     ...
     * @return true if a selection has been made at the given coordinates.
     */
    protected boolean selectValueAt(JXSlider slider, int x, int y) {
         ...
         *
         * If a value is found to be contained within the given coords then that value is selected and true is returned
        ...
        SliderValueRenderer r = slider.getValueRenderer();
        if (r != null && sel != null) { // can't select a value if they aren't rendered
            ...
            if (selectGroup != null) { // don't clear selection
    ...
    private static class Actions extends org.jdesktop.swingx.plaf.UIAction {
        ...
        @SuppressWarnings({"ConstantConditions"}) // for testing purposes
    ...
    private static class Adapter implements SliderChangeListener, MouseListener, MouseMotionListener, FocusListener, ChangeListener, ActionListener {
        ...
        /**
         * Provides the repeated events needed for continuous scrolling when mouse pressing in the track.
         */
        private Timer scrollTimer;
        private long pressedOffset = 0;
        // used to determine the current direction of timed events.
