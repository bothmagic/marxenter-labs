...
/**
 * FastFileChooser is basically a JTextField used to type the path to a File. The JTextField has an auto-completion
 * feature similar to that of most IDEs.<p>
 * Upon Ctrl+SPACE, an auto-completion window appears that contains the
 * Files compatible with the path currently typed.<p>
 * If Ctrl+SPACE is hit while the auto-completion window is displayed, the common prefix of all possible Files is
 * appended to the path.<p>
 * @author weebib
 */
public class FastFileChooser extends JPanel {
	// Used to retrieve icons for a given file
	// I find it quite ridiculous that there is no simplier mechanism than this to retrieve a file icon
...
//-- Constructor ------
	public FastFileChooser(File aStartDirectory) {
		...
		theFileItemList = new JList(theFileItemListModel) {
			public int getVisibleRowCount() {
				return Math.min(theFileItemListModel.getSize(), 10);
			}
		};
		...
		theFileItemList.addKeyListener(new KeyAdapter() {
			public void keyPressed(KeyEvent e) {
				if (e.getKeyCode() == KeyEvent.VK_ENTER) {
					setNextFileItem((FileItem)theFileItemList.getSelectedValue());
				}
			}
		});
		...
		inputMap.put(KeyStroke.getKeyStroke(KeyEvent.VK_SPACE, InputEvent.CTRL_MASK), "controlEspace");
		inputMap.put(KeyStroke.getKeyStroke(KeyEvent.VK_HOME, InputEvent.CTRL_MASK), "home");
		inputMap.put(KeyStroke.getKeyStroke(KeyEvent.VK_END, InputEvent.CTRL_MASK), "end");
		...
		theTextField.addKeyListener(new KeyAdapter() {
			public void keyPressed(KeyEvent e) {
				if (e.isConsumed() || !isAutoCompleting()) return;
				switch (e.getKeyCode()) {
...
//-- Private methods ------
	private void setCurrentFile(File aFile) {
		...
		if (theCurrentFile.isDirectory() && !absolutePath.endsWith(File.separator)) {
	...
	private boolean isAutoCompleting() {
		if (thePopupWindow == null) return false;
	...
	private void updateFileItems() {
		...
		FileItem[] fileItems = retrieveFileItems(dir, name);
		...
		Point windowLocation = new Point(p.x + theRelativePosition.x, p.y + theRelativePosition.y);
	...
	private FileItem[] retrieveFileItems(File aDirectory, final String aName) {
		if (aDirectory == null || !aDirectory.isDirectory() || aName == null) return new FileItem[0];
		File[] files = aDirectory.listFiles(new FileFilter() {
			public boolean accept(File pathname) {
				return (pathname.getName().toLowerCase().startsWith(aName.toLowerCase()));
			}
		});
	...
	private void addFileItem(List<FileItem> someFileItems, File aFile, String aPrefix) {
		if (aFile == null || !aFile.exists()) return;
	...
	private void placePopupWindow() {
		if (theRelativePosition == null) return;
		Point p = theTextField.getLocationOnScreen();
		Point windowLocation = new Point(p.x + theRelativePosition.x, p.y + theRelativePosition.y);
		windowLocation = relocatePopupWindow(windowLocation, thePopupWindow);
		thePopupWindow.setLocation(windowLocation);
	...
	private void moveDown() {
		...
		theFileItemList.scrollRectToVisible(theFileItemList.getCellBounds(index, index));
	}
	...
	private void moveStart() {
		...
		theFileItemList.scrollRectToVisible(theFileItemList.getCellBounds(0, 0));
	}
	...
	private void moveEnd() {
		...
		theFileItemList.scrollRectToVisible(theFileItemList.getCellBounds(endIndex, endIndex));
	}
	...
	private void movePageUp() {
		...
		int newIndex = Math.max(0, current - Math.max(0, theFileItemList.getVisibleRowCount() - 1));
		theFileItemList.setSelectionInterval(newIndex, newIndex);
		theFileItemList.scrollRectToVisible(theFileItemList.getCellBounds(newIndex, newIndex));
	}
	...
	private void movePageDown() {
		...
		if (size < 1) return;
		int current = theFileItemList.getSelectedIndex();
		int newIndex = Math.min(size - 1, current + Math.max(0, theFileItemList.getVisibleRowCount() - 1));
		theFileItemList.setSelectionInterval(newIndex, newIndex);
		theFileItemList.scrollRectToVisible(theFileItemList.getCellBounds(newIndex, newIndex));
	}
	...
	private static Point relocatePopupWindow(Point aPoint, JWindow aWindow) {
		...
		Insets insets = Toolkit.getDefaultToolkit().getScreenInsets(aWindow.getGraphicsConfiguration());
		Dimension screenDim = Toolkit.getDefaultToolkit().getScreenSize();
		Dimension availableDim = new Dimension(screenDim.width - insets.left - insets.right,
											   screenDim.height - insets.top - insets.bottom);
		Point p = new Point(aPoint);
		if (windowDim.width < availableDim.width) {
			int rightDelta = (insets.left + availableDim.width) - (p.x + windowDim.width);
			if (rightDelta < 0) {
				p.x = Math.max(insets.left, p.x + rightDelta);
			}
		}
		if (windowDim.height < availableDim.height) {
			int bottomDelta = (insets.top + availableDim.height) - (p.y + windowDim.height);
			if (bottomDelta < 0) {
